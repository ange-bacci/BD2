...
SELECT CODE, MOY_TEST FROM NOTATION
WHERE MOY_TEST IN (
    SELECT MAX(MOY_TEST) FROM NOTATION
);

-- Q14

SELECT NOM_ET, MOY_TEST FROM NOTATION
...
SELECT DISTINCT NUM_PROF, CODE FROM ENSEIGNT
WHERE NUM_PROF IN (
    SELECT RESP FROM MODULE
    WHERE MODULE.CODE = ENSEIGNT.CODE
);

-- Q17

SELECT DISTINCT LIBELLE FROM (
    SELECT CODE FROM MODULE
    MINUS
    SELECT MAT_SPEC FROM PROF
) R
JOIN MODULE ON MODULE.CODE = R.CODE;
...
SELECT DISTINCT NOM_ET, PRENOM_ET FROM ETUDIANT
JOIN NOTATION ON ETUDIANT.NUM_ET = NOTATION.NUM_ET
WHERE MOY_CC IS NULL OR MOY_TEST IS NULL;

SELECT DISTINCT NOM_ET, PRENOM_ET FROM ETUDIANT, NOTATION
WHERE ETUDIANT.NUM_ET = NOTATION.NUM_ET
AND (MOY_CC IS NULL OR MOY_TEST IS NULL);

SELECT DISTINCT NOM_ET, PRENOM_ET FROM ETUDIANT
WHERE NUM_ET IN (
    SELECT NUM_ET FROM NOTATION
    WHERE MOY_CC IS NULL OR MOY_TEST IS NULL
);
...
SELECT DISTINCT NOM_ET, PRENOM_ET FROM ETUDIANT
WHERE NUM_ET NOT IN (
    SELECT NUM_ET FROM ENSEIGNT
    WHERE NUM_PROF = (
        SELECT NUM_PROF FROM PROF
...
SELECT NOM_PROF, PRENOM_PROF FROM (
    SELECT NUM_PROF FROM PROF
    MINUS
    SELECT RESP FROM MODULE
) R
JOIN PROF ON PROF.NUM_PROF = R.NUM_PROF;
...
WHERE PROF.NUM_PROF = R.NUM_PROF;
