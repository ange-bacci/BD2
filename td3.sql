...
WHERE NOT EXISTS (
    SELECT * FROM ENSEIGNT E1
    WHERE NOT EXISTS (
        SELECT * FROM ENSEIGNT E2
        WHERE E2.NUM_ET = ETUDIANT.NUM_ET
        AND E2.NUM_PROF = E1.NUM_PROF
    )
    AND NUM_ET = 1102
);
...
SELECT NOM_ET, PRENOM_ET FROM ENSEIGNT
JOIN ETUDIANT ON ETUDIANT.NUM_ET = ENSEIGNT.NUM_ET
WHERE CODE = (
    SELECT CODE FROM MODULE
    WHERE LIBELLE = 'CONCEPTION DE SI'
)
GROUP BY NOM_ET, PRENOM_ET HAVING COUNT(DISTINCT ENSEIGNT.NUM_PROF) = (
    SELECT COUNT(DISTINCT NUM_PROF) FROM ENSEIGNT
    WHERE CODE = (
        SELECT CODE FROM MODULE
        WHERE LIBELLE = 'CONCEPTION DE SI'
    )
...
WHERE NOT EXISTS (
    SELECT * FROM ENSEIGNT E1
    JOIN MODULE ON E1.CODE = MODULE.CODE
    WHERE NOT EXISTS (
        SELECT * FROM ENSEIGNT E2 
        WHERE E2.NUM_ET = ETUDIANT.NUM_ET
        AND E2.NUM_PROF = E1.NUM_PROF
    ) AND MODULE.LIBELLE = 'CONCEPTION DE SI'
);
